...
class Dataset_generation(Dataset):
    ...
    def load_npy(self):
        self.x_train = np.load("x_train_811.npy").reshape(-1, 51, 2)
        self.x_valid = np.load("x_valid_811.npy").reshape(-1, 51, 2)
        self.x_test = np.load("x_test_811.npy").reshape(-1, 51, 2)
...
class EncoderStack(nn.Module):
    ...
    def forward(self, x):
        x_stack = []
        for i_len, encoder in zip(self.inp_lens, self.encoders):  # 3个
