...
class Dataset_generation(Dataset):
    ...
    def load_npy(self):
        self.x_train = np.load(self.path + "x_train.npy").reshape(-1,51,3)
        self.x_valid = np.load(self.path + "x_valid.npy").reshape(-1,51,3)
        self.x_test = np.load(self.path + "x_test.npy").reshape(-1,51,3)
        self.y_train = np.load(self.path + "y_train.npy").reshape(-1,51)
        self.y_valid = np.load(self.path + "y_valid.npy").reshape(-1,51)
        self.y_test = np.load(self.path + "y_test.npy").reshape(-1,51)
    ...
    def return_dataloader(self):
        ...
        mean1, mean2, mean3, std1, std2, std3 = self.norm_for_train()
        self.norm_data(mean1, mean2, mean3, std1, std2, std3)
        print(mean1, mean2, mean3, std1, std2, std3)
...
class Exp():
    def __init__(self, enc_in, c_out,  d_model, n_heads, e_layers,
        ...
        self.model = Informer(enc_in,  c_out,
                                   d_model, n_heads, e_layers,  d_ff,
                                   dropout, activation,distil).to(self.device)
        self.checkpoints_path = './checkpoint/NCM523/Attention'
