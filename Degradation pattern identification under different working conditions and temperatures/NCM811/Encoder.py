...
class Dataset_generation(Dataset):
    ...
    def load_npy(self):
        self.x_train = np.load("x_train_811.npy").reshape(-1, 51, 3)
        self.x_valid = np.load("x_valid_811.npy").reshape(-1, 51, 3)
        self.x_test = np.load("x_test_811.npy").reshape(-1, 51, 3)
        self.y_train = np.load("y_train_811.npy").reshape(-1, 51)
        self.y_valid = np.load("y_valid_811.npy").reshape(-1, 51)
        self.y_test = np.load("y_test_811.npy").reshape(-1, 51)
...
class Exp():
    ...
    def predict(self, test_loader, data_name):
        ...
        # new_preds, new_trues = self.deal_length(preds.T, trues.T)
